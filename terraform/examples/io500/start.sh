...
run_terraform() {
  ...
  terraform apply -input=false -parallelism=60 tfplan
  popd
}
...
waitforjobs() {
  while test $(jobs -p | wc -w) -ge "$1"; do wait -n; sleep 1; done
}
  ...
  if ! gcloud compute instances describe "${DAOS_FIRST_CLIENT}" \
    ...
    for node in ${ALL_NODES}; do
      ...
      gcloud compute instances add-metadata "${node}" \
        --project="${TF_VAR_project_id}" \
        --zone="${TF_VAR_zone}" \
        --metadata enable-oslogin=FALSE,enable-guest-attributes=TRUE \
